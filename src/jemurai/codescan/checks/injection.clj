...
(defn injectable? [line]
  (if
   (or
    ...
    (if (re-find #"\(read-string" line) true false) ; clojure
    (if (re-find #"clojure.java.shell" line) true false) ; clojure
    (if (re-find #"import \"os/exec\"" line) true false)  ; golang
    
; These create a lot of false positives.
...
(defn find-injection [root]
  ...
  (metafo (str "Found "
			...
			;	(println file)
                    (count (filter injectable? (line-seq rdr)))))) " potential injection points.")))
